...
def EvaluateBLINK(in_args):
    ...
    config = {
        ...
        "interactive": False,
        "top_k": in_args.topk,
        # "biencoder_model": "models/original_models/biencoder/epoch_7/pytorch_model.bin",
        # "biencoder_config": "models/original_models/biencoder/training_params.json",
        "biencoder_model": "models/exp3_original_model_74574_seed/biencoder/epoch_9/pytorch_model.bin",
        "biencoder_config": "models/exp3_original_model_74574_seed/biencoder/training_params.json",
        "entity_catalogue": "models/entity.jsonl",
        "entity_encoding": "models/original_embeddings/all_entities.encoding",
        "candidate_pool": "models/original_embeddings/cand_pool.t7",
        # "entity_encoding": "models/all_entities.encoding",
        # "candidate_pool": "models/cand_pool.t7",
        # "crossencoder_model": "models/crossencoder/epoch_5/pytorch_model.bin",
        "crossencoder_model": "models/original_models/crossencoder/epoch_1/pytorch_model.bin",
        "crossencoder_config": "models/original_models/crossencoder/training_params.json",
    ...
    if in_args.nyt:
        with io.open('data/nyt/new_nyt.jsonl', mode="r", encoding="utf-8") as file:
            for line in file:
                data_to_link.append(json.loads(line.strip()))
        print("Data Loaded", len(data_to_link))
        _, _, _, _, _, predictions, scores, = main_dense.run(args, None, *models, test_data=data_to_link, device='cuda',
                                                             alternate_mapping=(
                                                                 ...
                                                                 t2sub_events, t2sub_sections, t2date))
        for idx, data in enumerate(data_to_link):
            data['sub_events'] = data_to_link[idx]['sub_events']
            data['sub_sections'] = data_to_link[idx]['sub_sections']
            data['year'] = data_to_link[idx]['year']
        ...
        for t_idx, title in enumerate(clusters):
            for mention in clusters[title]:
                mention['sub_events'] = data_to_link[idx]['sub_events']
                mention['sub_sections'] = data_to_link[idx]['sub_sections']
                mention['year'] = data_to_link[idx]['year']
...
def WikiAccuracy(args, is_recall=False):
    # exp1_info_both_bi_cross
    # exp1_info_both_bi_cross
    # exp2_info_cross
    # exp3_info_bi
    # original_results
    infile = open('out/original_74574_seed/{}_result_top{}.json'.format(args.mode, args.topk), 'r')
    clusters = json.load(infile)

...
def process_mention_data(
    ...
    try:
        save_data_path = os.path.join("./out/", "new_train_p2.t7")
        torch.save(nn_data, save_data_path)
    except:
        save_data_path = os.path.join("./out/", "new_train_exp_p2.json")
        outfile = open(save_data_path, 'w')
        json.dump(nn_data, outfile)

    try:
        save_data_path = os.path.join("./out/", "backup_new_train_p2_cand_data.json")
        outfile = open(save_data_path, 'w')
        json.dump(backup_data, outfile)
    except:
        save_data_path = os.path.join("./out/", "backup_new_train_p2_cand_data.txt")
        outfile = open(save_data_path, 'w')
...
if __name__ == "__main__":
    ...
    train_samples = train_samples[15000:30000]

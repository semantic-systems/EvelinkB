...
wiki_wiki = wikipediaapi.Wikipedia('EvelinkB4 (evelinkb@gmail.com)', 'en')
...
def save_data(t2sub, t2sections, t2year):
    try:
        with open('./newdata/subevents/sub_event_p4_1.json', 'w') as f:
            json.dump(t2sub, f)
    except:
        save_data_path = os.path.join("./newdata/subevents/", "sub_event_p4_1.txt")
        outfile = open(save_data_path, 'w')
        outfile.write(f'{t2sub}')
        outfile.close()

    try:
        with open('./newdata/subsections/sub_sections_p4_1.json', 'w') as f:
            json.dump(t2sections, f)
    except:
        save_data_path = os.path.join("./newdata/subsections/", "sub_sections_p4_1.txt")
        outfile = open(save_data_path, 'w')
        outfile.write(f'{t2sections}')
        outfile.close()

    try:
        with open('./newdata/date/title_year_p4_1.json', 'w') as f:
            json.dump(t2year, f)
    except:
        save_data_path = os.path.join("./newdata/date/", "title_year_p4_1.txt")
        outfile = open(save_data_path, 'w')
